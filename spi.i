...
void SPI_Init()
{
	...
	(*((volatile unsigned char *) 0xE002000C)) = 0x1E;  
	(*((volatile unsigned short*) 0xE0020000)) = 0x824;  
	 
	reqDioFunction(22,0);  
	reqDioDirection(22,0);  
	reqDioOutput(22, 1);  
}
...
unsigned short SPI_Send(spi_comm_t *dataStruct)
{
	...
	reqDioOutput((dataStruct->chipSelectPin + 21), 0);  
	 
	for(index = 0; index < (dataStruct->numberOfBytes); index++)
	{
		 
		(*((volatile unsigned short*) 0xE0020008)) = *(dataStruct->pTxBuffer);
		 
		do
		{
			saveStatusReg = (*((volatile unsigned char *) 0xE0020004));
		} while(saveStatusReg == 0);
		 
		if(((*((volatile unsigned char *) 0xE0020004)) & 0x80) != 0)  
		{
			retValue++;
			*(dataStruct->pRxBuffer+index) = (*((volatile unsigned short*) 0xE0020008));
		}
		else if((saveStatusReg & 0x40) != 0)  
		{
			*(dataStruct->pRxBuffer) = (*((volatile unsigned short*) 0xE0020008));
			break;
		}
	...
	reqDioOutput((dataStruct->chipSelectPin + 21), 1);  
	return retValue;  
}
